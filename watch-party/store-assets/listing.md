...
Paste these into the matching fields in the Developer Dashboard.
...
## Remote code
```
No. The PeerJS library is bundled inside the extension package; no remote code is
loaded or executed.
```

## Suggested screenshots to capture (real captures recommended)
1. A Netflix title playing with the Couch overlay showing 2–3 call tiles.
2. The popup's "Create a party" screen.
3. The popup's in-party view showing the invite code and members.
4. The overlay collapsed to a small bar (shows it stays out of the way).
```
Target size: 1280×800 (or 640×400). At least one required; 3–5 recommended.
```
